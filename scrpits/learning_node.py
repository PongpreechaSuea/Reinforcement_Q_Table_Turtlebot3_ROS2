...
import sys
path_file = os.getcwd()
DATA_PATH = path_file.replace("scrpits","Data/")
MODULES_PATH = path_file
...
# Episode parameters
MAX_EPISODES = 400   # 400
MAX_STEPS_PER_EPISODE = 500     # 500
MIN_TIME_BETWEEN_ACTIONS = 0.0  # 1.0

# Learning parameters
ALPHA = 0.3  #0.5 ค่าคะแนนปัจจุบัน
GAMMA = 2.0  #0.9 ค่าคะแนนในอนาคต

T_INIT = 15 #25
T_GRAD = 0.95 #0.95
T_MIN = 0.001 #0.001

EPSILON_INIT = 0.9 #0.9
EPSILON_GRAD = 0.96 #0.96
EPSILON_MIN = 0.05 #0.05

# 1 - Softmax , 2 - Epsilon greedy
EXPLORATION_FUNCTION = 1 # 1
...
class LearningNode(Node):
    def __init__(self):
        ...
        self.timer_period = .1 # seconds .5
        self.timer = self.create_timer(self.timer_period, self.timer_callback)
        self.reset = self.create_client(Empty, '/reset_simulation')
        self.setPosPub = self.create_publisher(ModelState, 'gazebo/set_model_state', 10) #10
        self.velPub = self.create_publisher(Twist, 'cmd_vel', 10) #10
    ...
    def timer_callback(self):
            ...
            if step_time > MIN_TIME_BETWEEN_ACTIONS:
                ...
                if self.episode > MAX_EPISODES:
                    ...
                    print(sim_time_h , real_time_h)
                    print(sim_time_m , real_time_m)
                    print(sim_time_s , real_time_s)
