...
MAX_LIDAR_DISTANCE = 3.0  # 1.0
COLLISION_DISTANCE = 0.28 # 0.28 LaserScan.range_min = 0.1199999 , 0.14
NEARBY_DISTANCE = 0.45  # 0.45

ZONE_0_LENGTH = 0.4   # 0.4
ZONE_1_LENGTH = 0.7   # 0.7

ANGLE_MAX = 360 - 1   # 360 - 1
ANGLE_MIN = 1 - 1   # 1 - 1
HORIZON_WIDTH = 75   # 75
...
#     print(len(lidar[(ANGLE_MIN + HORIZON_WIDTH):(ANGLE_MIN):-1]))
#     print(len(W * lidar_horizon))
...
# Check - crash
def checkCrash(lidar): #================================================================
    lidar_horizon = np.concatenate((lidar[(ANGLE_MIN + 15):(ANGLE_MIN):-1],lidar[(ANGLE_MAX):(ANGLE_MAX - 15):-1]))
    W = np.linspace(1.2, 1, len(lidar_horizon) // 2)
    W = np.append(W, np.linspace(1, 1.2, len(lidar_horizon) // 2))
    if np.min( W * lidar_horizon ) < COLLISION_DISTANCE:
        return True
...
def checkCrash_B(lidar): #================================================================
    lidar_horizon = lidar[200:160:-1]
    W = np.linspace(1.2, 1, len(lidar_horizon) // 2)
    W = np.append(W, np.linspace(1, 1.2, len(lidar_horizon) // 2))
    if np.min( W * lidar_horizon ) < 0.35:
        return True
    else:
        return False      
    
def checkCrash_R(lidar): #================================================================
    lidar_horizon = lidar[100:80:-1]
    W = np.linspace(1.2, 1, len(lidar_horizon) // 2)
    W = np.append(W, np.linspace(1, 1.2, len(lidar_horizon) // 2))
    if np.min( W * lidar_horizon ) < 0.30:
        return True
    else:
        return False       

def checkCrash_L(lidar): #================================================================
    lidar_horizon = lidar[280:260:-1]
    W = np.linspace(1.2, 1, len(lidar_horizon) // 2)
    W = np.append(W, np.linspace(1, 1.2, len(lidar_horizon) // 2))
    if np.min( W * lidar_horizon ) < 0.30:
        return True
...
# MAX_LIDAR_DISTANCE = 9.0  # 1.0
# COLLISION_DISTANCE = 0.28 # LaserScan.range_min = 0.1199999 , 0.14
# NEARBY_DISTANCE = 0.45  # 0.45
