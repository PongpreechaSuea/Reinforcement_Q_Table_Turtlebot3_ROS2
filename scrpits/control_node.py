...
import sys
cwd = os.getcwd()
DATA_PATH = f'{cwd.replace("scrpits","Data/")}'
MODULES_PATH = f'{cwd}/'
...
if REAL_ROBOT:
    ...
    Y_INIT = 0.0
    THETA_INIT = 0.0
    XX_GOAL = [-1.0,0.0,1.5,0.0,-2.0]
    YY_GOAL = [-2.0,-2.0,1.3,1.5,-0.5]
    print(XX_GOAL , YY_GOAL)
    THETA_GOAL = 0.0
    ...
    def timer_callback(self):
        ...
        if step_time > MIN_TIME_BETWEEN_ACTIONS:
            ...
            if not self.robot_in_pos:
                ...
                crash_B = checkCrash_B(lidar)
                crash_R = checkCrash_R(lidar)
                crash_L = checkCrash_L(lidar)
                object_nearby = checkObjectNearby(lidar)
                goal_near = checkGoalNear(x, y, self.X_GOAL, self.Y_GOAL)
                enable_feedback_control = True

                # Stop the simulation
                if crash_B:
                    print("crashB")
                    self.velPub = self.create_publisher(Twist, 'cmd_vel', 0)
                    robotTurn(self.velPub) 
                
                # if crash_L:
                #     print("crashL")
                #     self.velPub = self.create_publisher(Twist, 'cmd_vel', 1)
                #     robotTurnRight(self.velPub)

                # if crash_R:
                #     print("crashL")
                #     self.velPub = self.create_publisher(Twist, 'cmd_vel', 1)
                #     robotTurnLeft(self.velPub)

                if crash:
                    print("crashA")
                    self.velPub = self.create_publisher(Twist, 'cmd_vel', 3)
                    robotTurn(self.velPub)
                    if crash_B:
                        print("crashB")
                        self.velPub = self.create_publisher(Twist, 'cmd_vel', 0)
                ...
                # Feedback control algorithm
                    ...
                    status = robotFeedbackControl(self.velPub, x, y, theta, self.X_GOAL, self.Y_GOAL, radians(THETA_GOAL))
                ...
                text = text + '\r\nx :       %.2f -> %.2f [m]' % (x, self.X_GOAL)
                text = text + '\r\ny :       %.2f -> %.2f [m]' % (y, self.Y_GOAL)
...
def main(args=None):
    ...
    for X_GOAL,Y_GOAL in zip(XX_GOAL,YY_GOAL):
        rclpy.init(args=args)        
        movebase_publisher = ControlNode(X_GOAL,Y_GOAL)
        try:
            # print(movebase_publisher)
            rclpy.spin(movebase_publisher)
        except SystemExit:                 # <--- process the exception 
            rclpy.logging.get_logger("Quitting").info('Done')
            # break
        print("RUN1 " , X_GOAL,Y_GOAL)
